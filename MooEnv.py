...
class MooSCH:
    ...
    def obj(self, u):
        x1 = u[0] * (self.x_up - self.x_low) + self.x_low
        x2 = u[1] * (self.x_up - self.x_low) + self.x_low
        # Define the two objective functions using x1 and x2
        f1 = (x1 - self.a) ** 2
        f2 = (x2 - self.b) ** 2
...
class MooTestSettingA:
    ...
    def reward_value(self, elite_list, obj_value):
        ...
        if if_dominate_others:
            # print("wow,you are really dominated",num_dominated,elite_list,obj_value)
            for i in range(len(elite_list) - 1, -1, -1):  # Iterate backwards
                if np.array_equal(elite_list[i], obj_value):
                    del elite_list[i]  # Remove the matched element
                    break  # Exit the loop after removing the first match from the end
            # print('if_dominate_others, reward ', reward)
            return reward
        elif if_dominated:
            # print('if_dominated ', reward)
            return reward
        else:
